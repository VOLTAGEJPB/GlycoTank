(function(){
  ...
  function wantForce(){
    ...
    return q.indexOf("intro=1")>-1 || q.indexOf("resetintro=1")>-1;
  }
  var KEY="glycotank_onboard_seen_v2";
  ...
  function hideStraysOutsideOverlay(){
    try{
      var stray = document.querySelectorAll(".onSlides, .onSlide, #onBack, #onNext");
      for(var i=0;i<stray.length;i++){
        var el=stray[i], p=el, inside=false;
        while(p){ if(p.id==="onboardOverlay"){ inside=true; break; } p=p.parentElement; }
      ...
      '<div class="onboard" role="dialog" aria-labelledby="onTitle" aria-modal="true">'+
        ...
        '<div class="onSlides">'+
          '<div class="onSlide active" data-i="0"><div class="onBody">'+
            '<p><strong>What is Glyco?</strong> It is your estimated glycogen “fuel tank,” in grams.</p>'+
            '<ul>'+
              ...
              '<li>Low tank -> go easier or add carbs. High tank -> you are primed to push.</li>'+
            '</ul>'+
          '</div></div>'+
          ...
          '<div class="onSlide" data-i="2"><div class="onBody">'+
            ...
            '<ul>'+
              ...
              '<li>Favorites: one-tap meals & sessions.</li>'+
  ...
  function showFirstRun(){
    ...
    if(wantForce()){ try{ localStorage.removeItem(KEY); }catch(_){ } seen=null; }
  ...
  function init(){
    hideStraysOutsideOverlay();
    ensureMarkup();
    var s=slides(); if(s.length && !s.some(function(el){return el.classList.contains("active");})){ s[0].classList.add("active"); }
    setStep(activeIndex());
    bindControls();
    showFirstRun();                 // show on both web and app
    window.GT_showIntro = function(){ try{ localStorage.removeItem(KEY); }catch(_){ } showFirstRun(); };
  }
